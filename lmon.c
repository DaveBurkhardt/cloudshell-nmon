...
#define P_A7SPEED	9	/* one more than the max */
#define P_A15SPEED	10	/* one more than the max */
#define P_CPUTEMP	11	/* one more than the max */
#define P_NUMBER	12	/* one more than the max */
...
void proc_init()
{
    ...
    proc[P_A7SPEED].filename = "/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq";
    proc[P_A15SPEED].filename = "/sys/devices/system/cpu/cpu4/cpufreq/scaling_cur_freq";
    proc[P_CPUTEMP].filename = "/sys/class/thermal/thermal_zone0/temp";
}
...
*/
	...
	if (p->dk[i].dk_xfers > 0 && strstr(&p->dk[i].dk_name[0],"boot") == NULL )
	    i++;
    }
    if (reread) {
	fclose(fp);
...
void proc_mem()
{
    ...
    p->mem.memavailable = proc_mem_search("MemAvailable");
...
void init_pairs()
{
    ...
    COLOUR init_pair((short) 13, (short) 0, (short) 5);	/* Magenta */
}
...
#define LSBLK_NO_TYPE		/* define this to work around missing --output TYPE feature */
	...
	padxu4 = newpad(MAXROWS, MAXCOLS);
	padfree = newpad(MAXROWS, MAXCOLS);
...
*/
	...
	if (cursed) {		/* Top line */
	  ...
	  //	mvprintw(x, 15, "[H for help]");
	    mvprintw(x, 2, "Hostname [ %s ]", hostname);
...
*/
	...
	if (show_help && cursed) {
	    ...
	    mvwprintw(padhelp, 8, 1,
                      "M = MHz by thread & CPU               | x = Xu4 CPU temp/A7 A15 mhtz");
#ifdef NVIDIA_GPU
	    ...
	    mvwprintw(padhelp, 13, 1,
                      "f = Memory stats similar to 'free'    | b = black & white mode");
...
*/
	if (show_xu4) {
	    if (cursed) {
                proc_read(P_CPUTEMP);
                proc_read(P_A7SPEED);
                proc_read(P_A15SPEED);
		BANNER(padxu4, "XU4");
		COLOUR wattrset(padxu4, COLOR_PAIR(0));
		mvwprintw(padxu4, 1, 5, "Temp",atoi(proc[P_CPUTEMP].line[0])/1000);
		mvwprintw(padxu4, 1, 20, "A15 @ %d Mhz",atoi(proc[P_A15SPEED].line[0])/1000);
		mvwprintw(padxu4, 1, 40, "A7 @ %d Mhz",atoi(proc[P_A7SPEED].line[0])/1000);
                if (atoi(proc[P_CPUTEMP].line[0]) > 70000) {
		COLOUR wattrset(padxu4, COLOR_PAIR(8));
               } else {
		COLOUR wattrset(padxu4, COLOR_PAIR(2));
               }
	       mvwprintw(padxu4, 1, 11, "%d C",atoi(proc[P_CPUTEMP].line[0])/1000);
	        DISPLAY(padxu4, 2);
		COLOUR wattrset(padxu4, COLOR_PAIR(6));
            }
        }
	...
	if (show_free) {
		...
		//mvwprintw(padfree, 1, 20, "A15 @ %d Mhz",atoi(proc[P_A15SPEED].line[0])/1000);
	        DISPLAY(padfree, 2);
		COLOUR wattrset(padfree, COLOR_PAIR(5));
            }
        }
...
/* DEBUG inject steal       cpu_steal = cpu_sys; */
			...
			if (i == 0)
			    mvwprintw(padsmp, 2 + i, 27,
				      "| Please wait gathering CPU statistics");
			else
			    mvwprintw(padsmp, 2 + i, 27, "|");
			mvwprintw(padsmp, 2 + i, 77, "|");
		    } else {
#ifdef POWER
		...
		CURSE mvwprintw(padsmp, i + 2, 0, cpu_line);
#ifdef POWER
...
*/
		...
		COLOUR wattrset(padnet, COLOR_PAIR(2));
		mvwprintw(padnet, 2 + i, PIN, "%6.1f",
			  IFDELTA(if_ipackets));
		COLOUR wattrset(padnet, COLOR_PAIR(3));
		mvwprintw(padnet, 2 + i, POUT, "%7.1f",
			  IFDELTA(if_opackets));
		COLOUR wattrset(padnet, COLOR_PAIR(2));
		mvwprintw(padnet, 2 + i, SIN, "%6.1f",
			  IFDELTA_ZERO(if_ibytes, if_ipackets));
		COLOUR wattrset(padnet, COLOR_PAIR(3));
		mvwprintw(padnet, 2 + i, SOUT, "%6.1f",
			  IFDELTA_ZERO(if_obytes, if_opackets));
		COLOUR wattrset(padnet, COLOR_PAIR(2));
		mvwprintw(padnet, 2 + i, PKIN - 4, "%7.1f",
			  net_read_peak[i]);
		COLOUR wattrset(padnet, COLOR_PAIR(3));
		mvwprintw(padnet, 2 + i, PKOUT, "%7.1f",
			  net_write_peak[i]);
		}
	    }
